  ...
  return (
    <PageFrame navItems={enContent.nav} langToggle={{ href: '/contacto/', label: 'ES' }}>
      <header className="mx-auto w-full max-w-[375px] px-4 pt-12 pb-2 bg-brand-100">
        <p className="stamp-label mb-2">Contact</p>
        <p className="font-serif text-2xl font-black text-brand-900">Jabugo Bodega</p>
        <p className="mt-1 text-[13px] text-brand-500 tracking-[0.08em]">{contactoContent.hero.location}</p>
        <div className="mt-6 flex flex-col gap-3 pb-10">
          {contactoContent.hero.ctas.map((cta, idx) => (
            <CTAButton key={cta.label} href={cta.href} variant={idx === 0 ? 'primary' : 'ghost'}>
              {cta.label}
            </CTAButton>
          ))}
        </div>
