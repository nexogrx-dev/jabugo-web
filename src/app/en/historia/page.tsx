...
export default function HistoriaEnPage() {
  return (
    <PageFrame navItems={enContent.nav} langToggle={{ href: '/historia/', label: 'ES' }}>
      <PageHeader label="History" title={historiaContent.sections[0].title} description={historiaContent.meta.description} />
      {historiaContent.sections.map((section, idx) => (
        <div key={section.id}>
          <section id={section.id} className="mx-auto w-full max-w-[375px] px-4 py-10">
            ...
            {section.cta?.href && (
              <div className="mt-8">
                <CTAButton href={section.cta.href} variant="primary">{section.cta.label}</CTAButton>
              </div>
            )}
          </section>
