...
function PageHeader({ label, title, description }: { label: string; title: string; description?: string }) {
  return (
    <header className="mx-auto w-full max-w-[375px] px-4 pt-12 pb-2">
      ...
      <h1 className="mt-2 font-serif text-[2.25rem] font-black leading-[1.1] text-brand-900">{title}</h1>
      {description && <p className="mt-3 text-sm leading-6 text-brand-700">{description}</p>}
      <Divider />
    </header>
  );
}
...
export default function CultureEnPage() {
  return (
    <PageFrame navItems={enContent.nav} langToggle={{ href: '/cultura-iberico/', label: 'ES' }}>
      <PageHeader label="Iberian guide" title="Iberian culture" description={culturaContent.meta.description} />
      <nav className="mx-auto w-full max-w-[375px] px-4 pb-4" aria-label="Index">
        <ol className="space-y-2">
          {culturaContent.sections.map((s, i) => (
            <li key={s.id}>
              <a href={`#${s.id}`} className="flex items-center gap-3 text-sm font-semibold text-brand-700 hover:text-brand-900 transition-colors">
      ...
      <Divider />
      {culturaContent.sections.map((section, idx) => (
        <div key={section.id}>
          <section id={section.id} className="mx-auto w-full max-w-[375px] px-4 py-10">
            <p className="stamp-label mb-3">{'0' + (idx + 1)}</p>
            <TextBlock title={section.title} paragraphs={section.paragraphs} quote={section.quote} />
            {section.cta?.href && (
              <div className="mt-8">
                <CTAButton href={section.cta.href} variant="ghost">{section.cta.label}</CTAButton>
              </div>
            )}
          </section>
          {idx < culturaContent.sections.length - 1 && <Divider />}
        </div>
      ))}
    </PageFrame>
  );
